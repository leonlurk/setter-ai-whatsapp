...
app.get("/users/:userId/active-agent", async (req, res) => {
  ...
  try {
    ...
    res.json({
      success: true,
      activeAgentId: docSnap.data().active_agent_id || null,
    });
  } catch (err) {
    console.error(
      "[Server][Firestore Error] Error obteniendo agente activo:",
      err
    );
    return res.status(500).json({ success: false, message: "Error interno." });
  }
...
app.post("/api/instagram/generate-reply", async (req, res) => {
  ...
  console.log(
    `[Server][Instagram] Received request for ${instagramUserId}, thread ${threadId}. Message: "${message.substring(
      0,
      50
  ...
  try {
    // TODO: Potentially load agentConfig here based on agentId or instagramUserId
    // For now, passing null or a default config to buildPromptForInstagram
    let agentConfigToUse = null;
    // Example: if (agentId && fs.existsSync(path.join(__dirname, 'gemini-starters.json'))) {
    // const starters = JSON.parse(fs.readFileSync(path.join(__dirname, 'gemini-starters.json'), 'utf-8'));
    // agentConfigToUse = starters.find(s => s.id === agentId) || starters[0]; // or some default
    // }

    const promptContent = await buildPromptForInstagram(
      message,
      conversationHistory,
      instagramUserId,
      agentConfigToUse
    ...
    if (geminiReply) {
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to get a reply from Gemini." });
    }
  } catch (error) {
    console.error(
